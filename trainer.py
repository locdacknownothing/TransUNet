...
def trainer_retinal_vessel(args, model, snapshot_path):
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.loss_name == 'dice_ce':
                ...
                # ce_weights = calculate_ce_weights(label_batch, num_classes)
                # print(f"Calculated CE weights: {ce_weights.cpu().numpy()}")
                # focal_alpha = torch.tensor([0.2, 0.8]).cuda()
                focal_alpha = None  # Use default alpha in FocalLoss if not specified
                loss_dict = calculate_vessel_loss(outputs, label_batch, num_classes=num_classes, alpha=focal_alpha)
            else:
